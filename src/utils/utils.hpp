...
inline void savePointCloudsAsPLY(
        const std::vector<sensor_msgs::PointCloud2> &pointclouds,
        const std::string &output_path,
        int32_t file_idx) {
    const std::filesystem::path out_path(output_path);
    if (!std::filesystem::is_directory(out_path)) {
    ...
    file_idx = 0;
    std::for_each(
            pointclouds.cbegin(), pointclouds.cend(), [&](const auto &pcl) {
                std::ofstream ply_out;
                const std::string filename = std::to_string(file_idx) + ".ply";
                ply_out.open(out_path / filename,
                             std::ios_base::out | std::ios_base::binary);
                ...
                for (const auto &point : pcl.data) {
                    for (auto val : point) {
                        ply_out.write(reinterpret_cast<char *>(&val),
                                      sizeof(val));
                    }
                }
            });
